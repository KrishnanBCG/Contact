...
export class MainComponent implements OnInit {
  ...
  sidebarcontact:boolean=false;
  sidebartags:boolean=false;
  ...
  togglesidebarcontact(){
    this.sidebarcontact=!this.sidebarcontact
  }
  ...
  onInputChange() {
    if (this.searchQuery != '') {
      // this.http.get<any>(`http://localhost:4000/search?name=${this.searchQuery}`).subscribe((res) => {
        this.http.get<any>(`https://stately-moonbeam-6cc505.netlify.app/.netlify/functions/app/search?name=${this.searchQuery}`).subscribe((res) => {
